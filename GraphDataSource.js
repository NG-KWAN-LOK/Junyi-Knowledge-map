...
GraphDataSource.prototype.initialGraphElement = function (callbackView) {
  ...
  var f1 = $.ajax({
    type: "GET",
    url: "./new_data.csv", //"./shujun_in_line_20180102.csv", // https://dl.dropbox.com/s/c5ib14yaw6hmh2k/shujun_fraction_in_line_v2.csv
    dataType: "text",
  ...
  cg = $.when(f1);
  cg.done(function () {
    var cgGraphElement = self.createCgGraphElement(
      relationList,
      descriptionList
    );
...
GraphDataSource.prototype.createCgGraphElement = function (
  relationList,
  descriptionList
) {
...
GraphDataSource.prototype.updateData = function (
  urlList,
...
GraphDataSource.prototype.getDisplayNodeList = function () {
  ...
  if (this.displayJunyiResult && this.curDate === "all") {
    ...
    filterData = studentData.filter(function (result) {
      var typeMatched = self.displayJunyiResult
        ? true
        : result["type"] === "remedial";
      var timeMatched =
        self.curDate === "all"
          ? true
          : result["date"] === self.curDate || result["type"] === "junyi"; // display specific date of remedial result and all junyi result
  ...
  for (var i = 0; i < filterData.length; i++) {
    ...
    if (result["type"] === "remedial") {
      var name = this.cgLevel
        ? result["name"]
        : this.cgToSection(result["name"]);
      ...
      var needUpdate = remedialData[name]["date"]
      ...
      var name = this.cgLevel
        ? this.exToCg(result["name"])
        : this.exToSection(result["name"]);
      if (!(name in junyiData)) {
        ...
        if (
          !junyiData[name]["date"] ||
          result["date"] > junyiData[name]["date"]
        ) {
      ...
      var curDesc =
        descDict[result["status"]] + " " + this.getExName(result["name"]);
      if (result["date"]) {
        curDesc = result["date"] + " " + curDesc;
      }
  ...
  for (var i = 0; i < nodeListTmp.length; i++) {
    ...
    if (this.searchStatus["not_learned"].indexOf(node["group"]) !== -1) {
      nodeListTmp = this.guessNodeGroup(
        nodeId,
        nodeListTmp,
        this.curEdgeList,
        "forward"
  ...
  for (var i = 0; i < nodeListTmp.length; i++) {
    ...
    if (node["group"] === "fit" || node["group"] === "easy") {
      ...
      nodeListTmp = this.recursiveFillLearnedNode(
        nodeId,
        nodeDate,
        nodeListTmp,
        this.curEdgeList,
...
GraphDataSource.prototype.combineAndSummary = function (
  remedialData,
  junyiData
) {
  ...
  for (var name in junyiData) {
    ...
    if (!(name in displayData)) {
      ...
      displayData[name] = {
        date:
          remedialData[name]["date"] > junyiData[name]["date"]
            ? remedialData[name]["date"]
            : junyiData[name]["date"],
        desc: remedialData[name]["desc"] + junyiData[name]["desc"],
        satus:
          remedialData[name]["date"] > junyiData[name]["date"]
            ? remedialData[name]["status"]
            : junyiData[name]["status"],
      };
      // handle the conflict case
      if (
        (this.searchStatus["learned"].indexOf(remedialData[name]["status"]) ===
          -1 &&
          this.searchStatus["not_learned"].indexOf(
            remedialData[name]["status"]
          ) === -1) ||
        (this.searchStatus["not_learned"].indexOf(
          remedialData[name]["status"]
...
GraphDataSource.prototype.guessNodeGroup = function (
  nodeId,
  _nodeList,
  _edgeList,
  way
    ...
    if (
      ...
      for (var j = 0; j < _nodeList.length; j++) {
        ...
        if (_node["id"] === guessNodeId && !_node["group"]) {
          ...
          if (
            (way === "backward" && _node["group"] === "guessX") ||
...
GraphDataSource.prototype.recursiveFillLearnedNode = function (
  _nodeId,
  _date,
  _curNodeList,
  _curEdgeList,
      ...
      if (_rememberNumber < 0 || (prevDate && prevDate > _date)) {
        ...
        for (var nodeIdx = 0; nodeIdx < _curNodeList.length; nodeIdx++) {
          if (_curNodeList[nodeIdx]["id"] === prevNodeId) {
            if (
              this.searchStatus["not_learned"].indexOf(
                _curNodeList[nodeIdx]["group"]
              ) !== -1 ||
              this.searchStatus["not_sure"].indexOf(
                _curNodeList[nodeIdx]["group"]
              ) !== -1
            ) {
        ...
        _curNodeList = this.recursiveFillLearnedNode(
          prevNodeId,
          _date,
          _curNodeList,
          _curEdgeList,
...
GraphDataSource.prototype.getExBySection = function (section) {
  ...
  var matchedExercise = this.liveExercise.filter(function (exercise) {
    var matched =
      exercise["chapter_title"] === chapterName &&
      exercise["difficulty"] === "基礎";
    if (sectionName) {
...
GraphDataSource.prototype.getCurGraphElement = function (theme) {
  if (!theme) {
    return [this.curNodeList, this.curEdgeList];
  }
  var currentNodeList = this.curNodeList.filter(function (item) {
    return item.theme === theme;
  });
  return [currentNodeList, this.curEdgeList];
};
...
GraphDataSource.prototype.setDisplayJunyiResult = function (
  displayJunyiResult
) {
...
GraphDataSource.prototype.getThemeList = function () {
  console.log("secNodeList", this.secNodeList);
  if (!this.secNodeList) {
    return;
  }
  const filteredArr = this.secNodeList
    .reduce((acc, current) => {
      if (
        !acc.find(function (item) {
          return item.theme === current.theme;
        })
      ) {
